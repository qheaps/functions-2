...
function redun(uArr) {
  console.log(`The new names array with all duplicate items removed is ${uArr}`)
}
...
// CODE HERE 
function getUserById(userArr, id, callback) {
  for (i = 0; i < userArr.length; i++) {
    if (id === userArr[i].id) {
      return callback(userArr[i])
    }
  }
}
